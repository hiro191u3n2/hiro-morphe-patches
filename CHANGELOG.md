...
* **SwiftKey Beta:** 9.13.16.4向けに3点を追加。GoogleIME用カタカナ語辞書45,686組（37,832読み）で「すいっち→switch」等の英語候補を追加します。日本語入力中の外部貼り付け・SwiftKeyクリップボード貼り付け後に、末尾の英語を再変換へ取り込む経路を保護。「レイアウトとキー」に日本語QWERTYの「123」と改行キーを交換するスイッチを追加しました（初期オフ、標準・片手配列対象、PC配列対象外）。SwiftKey用MPPはv1.3.0です。
